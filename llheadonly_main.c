...
processFasta(char *filename, double *timeTaken)
{
    ...
    do {
        ...
        if (status == 0) {
            ...
            // Create a new node for the current FASTA record
            LLvNode *newNode = llNewNode(fRecord.description, fRecord.sequence);
    ...
    // You can free the linked list when you're done with it
